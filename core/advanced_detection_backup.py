...
class GenuineThreatDetector:
    ...
    def analyze_network_threats(self, networks):
        """
        ...
        for network in networks:
            ...
            # Flag as high-risk if score >= 60 (conservative threshold)
            if enhanced_network.get('Threat_Score', 0) >= 60:
    ...
    def _analyze_mac_address(self, bssid):
        ...
        try:
            ...
            if first_octet & 0x02:  # Locally administered bit set
                score += self.thresholds['hardware']['locally_admin_penalty']
                reasons.append("Locally administered MAC address")
        except ValueError:
            pass
    ...
    def _analyze_temporal_behavior(self, bssid, current_time):
        ...
        # Check network lifespan
        lifespan = max(timestamps) - min(timestamps)
        if lifespan < self.thresholds['timing']['short_lifespan']:
            score += 20
            reasons.append(f"Short network lifespan ({int(lifespan/60)} minutes)")
        
        # Check for rapid on/off cycling
        if len(history) >= 4:
            time_gaps = []
            ...
            # Look for rapid cycling pattern
            short_gaps = [gap for gap in time_gaps if gap < self.thresholds['timing']['rapid_cycling']]
            if len(short_gaps) > 2:
                score += 15
                reasons.append("Rapid on/off cycling pattern")
        
        # Check for burst appearances (multiple detections in short time)
        recent_cutoff = current_time - 3600  # Last hour
        recent_appearances = [ts for ts in timestamps if ts > recent_cutoff]
        if len(recent_appearances) >= self.thresholds['timing']['burst_appearances']:
            score += 10
            reasons.append("Multiple appearances in short timeframe")
        ...
        for other_bssid in self.network_history.keys():
            if other_bssid != bssid:
                try:
                    ...
                    # Check if MACs are sequential (within 10 addresses)
                    if 1 <= diff <= 10:
                        sequential_count += 1
                except ValueError:
                    continue
    ...
    def _update_network_history(self, networks, current_time):
        """Update network history for behavioral analysis"""
        for network in networks:
            bssid = network.get('BSSID')
            if bssid:
